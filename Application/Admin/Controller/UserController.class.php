...
class UserController extends Controller {
    ...
    {    
        $userDB = M('user');
        $counts = $userDB->count();
        $Page       = new \Think\Page($counts,10);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        //$Page     = new \Think\VshowPage($counts,10);
        $show     = $Page->show();
        $userList = $userDB->field('id,username,truename,status,reg_date,last_login,last_ip')->limit($Page->firstRow.','.$Page->listRows)->select();
